...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if self not in QueenAnt.double:
            self.damage *=2
            QueenAnt.double.add(self)
        # END Problem 12
    ...
    def nearest_bee(self):
        ...
        while not location.is_hive:
            if count >= self.lower_bound:
                in_range = [bee for bee in location.bees if count <= self.upper_bound]
                if in_range:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health - amount <= 0: #fireant dies
            copy = self.place.bees[:]
            for bee in copy: #damage
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        if self.ant_contained is not None:
            self.ant_contained.action(gamestate)
        [bee.reduce_health(self.damage) for bee in list(self.place.bees)]
# END Problem 9
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if insect and not insect.is_waterproof:
            for i in insect:
                i.health = 0
        # END Problem 10
